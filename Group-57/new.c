    ...
    if(pid1 == 0){                                                                // shared memory is available in all child processes .
        ...
        if (pid_grandchild == 0) 
        {
            ...
            sem_wait(&(sh_ptr->ready_queue.sem)) ;
                ...
                {
                    printf("Entering ready_queue2 semaphore in submit_task\n");
                    sh_ptr->ready_queue1.enqueue(&(sh_ptr->ready_queue1) , address) ;
                    printf("leaving ready_queue2 semaphore in submit_task\n");
                }
                else if(priority == 3 )
                {
                    printf("Entering ready_queue semaphore in submit_task\n");
...
void simple_scheduler(){                                                                                               // open shared memory in simple_scheduler process .
    ...
    {   
        ...
        for(int j = 0 ;  j < sh_ptr->ncpu ; j ++)
        {
            ...
            else if (sh_ptr->ready_queue.front != sh_ptr->ready_queue.rear)
            {
                printf("hi\n") ;
            ...
            sem_post(&(sh_ptr->sem)) ;
                ...
                printf("Leaving run sem in simpleshed\n") ;
        ...
        while(j++ < cpu){       
                ...
                printf("In while\n") ;
                sem_wait(&(sh_ptr->running_queue.sem)) ;
                printf("Entered run_sem in simpleshed\n") ;
                task * t = sh_ptr->running_queue.dequeue(&(sh_ptr->running_queue)) ;
                printf("Leaving run_sem in simpleshed\n") ;
                sem_post(&(sh_ptr->running_queue.sem)) ;
                if(t == NULL)
                {
                ...
                sem_wait(&(sh_ptr->sem)) ;
                    ...
                    printf("stopped %d in scheduler after running dequeue\n" , t->pid) ;
...
{   // int dummy_main(int argc , char * argv[] ) later ..
    ...
    sh_ptr->ready_queue1.rear = 0  ;
    sh_ptr->ready_queue2.front = 0 ;
    sh_ptr->ready_queue2.rear = 0  ;
    sh_ptr->ready_queue3.front = 0 ;
    sh_ptr->ready_queue3.rear = 0  ;
