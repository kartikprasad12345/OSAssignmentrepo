    ...
    if(pid1 == 0){                                                                // shared memory is available in all child processes .
        ...
        if (pid_grandchild == 0) 
        {
            ...
            sem_wait(&(sh_ptr->ready_queue.sem)) ;
                ...
                sh_ptr->ready_queue.enqueue(&(sh_ptr->ready_queue), address) ;// &sh_ptr->process_table[0] is the address of the process we should not loose info until it is sent to termination queue
