...
typedef struct shm_t
{
    ...
    queue ready_queue1;
    queue ready_queue2;
    queue ready_queue3;                                                       // ready_queue->array[MAX_PID]  , ready_queue->size , ready_queue->front , ready_queue->rear
    ...
    if(pid1 == 0){                                                                // shared memory is available in all child processes .
        ...
        if (pid_grandchild == 0) 
        {
            ...
            sem_wait(&(sh_ptr->sem)) ;
                ...
                while(!sh_ptr->process_table[i1].flag == -1  )
                {                                                                            // -1 -> unused
                    n = (n+1) % MAX_PID ;
                    j ++ ;
                    if(j == MAX_PID)
        ...
        for(int i=4;i>=1;i--){
            if(i==4){
                while(s3 != r3 )
                {
                    printf("In here\n") ;
                    sem_wait(&(sh_ptr->readysem)) ;
                    task * t = sh_ptr->ready_queue3.array[i1] ;
                    sem_post(&(sh_ptr->readysem)) ;
                    sem_wait(&(sh_ptr->sem)) ;
            ...
            if(i==3){
                while(s2 != r2 )
                {
                    printf("In here\n") ;
                    sem_wait(&(sh_ptr->readysem)) ;
                    task * t = sh_ptr->ready_queue2.array[i1] ;
                    sem_post(&(sh_ptr->readysem)) ;
                    sem_wait(&(sh_ptr->sem)) ;
                    printf("flag in simple scheduler : %d  %d\n" , t->flag , t->pid) ;
                    printf("setting waiting time \n");
            ...
            if(i==2){
                while(s1 != r1 )
                {
                    printf("In here\n") ;
                    sem_wait(&(sh_ptr->readysem)) ;
                    task * t = sh_ptr->ready_queue1.array[i1] ;
                    sem_post(&(sh_ptr->readysem)) ;
                    sem_wait(&(sh_ptr->sem)) ;
                    printf("flag in simple scheduler : %d  %d\n" , t->flag , t->pid) ;
                    printf("setting waiting time \n");
            ...
            if(i==1){
                while(s != r  )
                {
                    printf("In here\n") ;
                    sem_wait(&(sh_ptr->readysem)) ;
                    task * t = sh_ptr->ready_queue.array[i1] ;
                    sem_post(&(sh_ptr->readysem)) ;
                    sem_wait(&(sh_ptr->sem)) ;
                    printf("flag in simple scheduler : %d  %d\n" , t->flag , t->pid) ;
                    printf("setting waiting time \n");
