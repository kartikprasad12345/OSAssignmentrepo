...
void submit_task(char * file_name,int priori)
{
    if(fork() == 0){   
        ...
        if (pid_grandchild == 0) 
        {
            ...
            sem_post(&(sh_ptr->sem)) ;
            printf("Here too 1\n");
            sem_wait(&(sh_ptr->readysem)) ;//take 4 ready sem more
            printf("Here too 2\n");
...
{   // int dummy_main(int argc , char * argv[] ) later ..
    ...
    while(j < MAX_PID){
        ...
        sh_ptr->ready_queue1.array[j] = NULL ;
        sh_ptr->ready_queue2.array[j] = NULL ;
        sh_ptr->ready_queue3.array[j] = NULL ;
    ...
    if(fork() == 0){   
        ...
        if (pid_grandchild == 0) 
        {
            ...
            sem_wait(&(sh_ptr->sem)) ;
                ...
                printf("Process %s pid %d start_time %ld flag : %d\n" , sh_ptr->process_table[n].file_name , sh_ptr->process_table[n].pid , sh_ptr->process_table[n].start_time.tv_sec, sh_ptr->process_table[n].flag) ;
            ...
            if(t->priori==1){
                ...
                sh_ptr->ready_queue.enqueue(&(sh_ptr->ready_queue), &sh_ptr->process_table[n]);
            }else if(t->priori==2){
                if(sh_ptr->ready_queue1.size  == 0)
                {
                    printf("posting null_sem in submit task\n") ;
                ...
                sh_ptr->ready_queue1.enqueue(&(sh_ptr->ready_queue1), &sh_ptr->process_table[n]);
            }else if(t->priori==3){
                if(sh_ptr->ready_queue2.size  == 0)
                {
                    printf("posting null_sem in submit task\n") ;
                ...
                sh_ptr->ready_queue2.enqueue(&(sh_ptr->ready_queue2), &sh_ptr->process_table[n]);
            }else if(t->priori==4){
                if(sh_ptr->ready_queue3.size  == 0)
                {
                    printf("posting null_sem in submit task\n") ;
                ...
                sh_ptr->ready_queue3.enqueue(&(sh_ptr->ready_queue3), &sh_ptr->process_table[n]);
            }
                //printf("Enter ready_queue semaphore in submit_task\n") ; 
                    // if(sh_ptr->ready_queue.size  == 0)
                    // {
            ...
            sem_wait(&(sh_ptr->sem)) ;
                ...
                sh_ptr->process_table[n].file_name = file_name ; 
...
{                                                                      // signal handler for scheduler process
            if(sig == SIGTERM){
                ...
                while(i < sh_ptr->termination_top)
                {
                    ...
                    printf("Process %s\n" , sh_ptr->termination_queue[i].file_name) ;
                    printf("Waiting Time %f\n" , sh_ptr->termination_queue[i].waiting_time) ;
                    long int  execution_time = (sh_ptr->termination_queue[i].termination_time.tv_sec - sh_ptr->termination_queue[i].start_time.tv_sec) *1000
                    + (sh_ptr->termination_queue[i].termination_time.tv_usec - sh_ptr->termination_queue[i].start_time.tv_usec) / 1000;
                //  in milliseconds . check for error when calculating execution time .
                    sem_post(&(sh_ptr->sem)) ;
...
void simple_scheduler(){                                                                                               // open shared memory in simple_scheduler process .
    ...
    {   printf("Waiting for post \n\n") ;
        ...
        sem_wait(&(sh_ptr->readysem));
            sh_ptr->ready_queue.display(&sh_ptr->ready_queue)    ;
        sem_post(&(sh_ptr->readysem)) ;                                                                           // wait for shell to submit a task
        printf("In scheduler\n\n") ;                                                                            // wait for shell to submit a tas                                                                                         // wait for shell to submit a task    
        ...
        for(int j = 0 ;  j < sh_ptr->ncpu ; j ++)
        {
            ...
            sem_wait(&(sh_ptr->readysem)) ;                                                                         // running
