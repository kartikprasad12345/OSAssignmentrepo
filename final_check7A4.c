...
//    void *start_addr = (void *)(((unsigned long)phdr[seg_index].p_vaddr+segments[seg_index].ifloaded*PAGE_SIZE));
    ...
    if (seg_index == 1) {
        cnt1++;
        alloc_addr = mmap((void *)((uintptr_t)phdr[seg_index].p_vaddr+ segments[seg_index].ifloaded*PAGE_SIZE), PAGE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_FIXED, fd, phdr[seg_index].p_offset+segments[seg_index].ifloaded*PAGE_SIZE);
        segments[seg_index].ifloaded++;
    } else {
        cnt1++;
        alloc_addr = mmap((void *)((uintptr_t)phdr[seg_index].p_vaddr+segments[seg_index].ifloaded*PAGE_SIZE), PAGE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_FIXED | MAP_ANONYMOUS, fd, phdr[seg_index].p_offset+segments[seg_index].ifloaded*PAGE_SIZE);
        segments[seg_index].ifloaded++;
    }
