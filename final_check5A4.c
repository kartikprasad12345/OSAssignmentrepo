...
void handle_segfault(int signum, siginfo_t *info, void *context) {
  printf("*******************\n");
    ...
    if(seg_index==1){
      cnt1++;
      segments[seg_index].all_mapped_addresses[segments[seg_index].ifloaded]=start_addr;
      segments[seg_index].ifloaded++;
      alloc_addr= mmap(start_addr, PAGE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC , MAP_PRIVATE|MAP_FIXED , fd, offset_from_start_of_file);
    }else{
      cnt1++;
      segments[seg_index].all_mapped_addresses[segments[seg_index].ifloaded]=start_addr;
      segments[seg_index].ifloaded++;

      alloc_addr= mmap(start_addr, PAGE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC , MAP_PRIVATE|MAP_FIXED|MAP_ANONYMOUS , fd, offset_from_start_of_file);
    }
