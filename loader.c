  ...
  for(int i=0;i<total_phdr;i++){
    if((pghdrs_list[i].temp.p_vaddr<seg_fault_addr) && (pghdrs_list[i].temp.p_vaddr+pghdrs_list[i].temp.p_memsz>=seg_fault_addr) && pghdrs_list[i].ifloaded==false){
      if(count_page_fault==0){
        mmap((void *)pghdrs_list[i].temp.p_vaddr, pghdrs_list[i].temp.p_memsz, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_FIXED,fd,pghdrs_list[i].temp.p_offset);
        pghdrs_list[i].ifloaded=true;
        count_page_fault++;
        break;
      }else{
        mmap((void *)pghdrs_list[i].temp.p_vaddr, pghdrs_list[i].temp.p_memsz, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_FIXED|MAP_ANONYMOUS,fd,pghdrs_list[i].temp.p_offset);
        pghdrs_list[i].ifloaded=true;
        break;
      }
    }
